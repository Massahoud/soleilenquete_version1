...
class _UserProfilState extends State<UserProfil> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final String userIdFromRoute = ModalRoute.of(context)?.settings.arguments as String;
      setState(() {
  ...
  void _loadUserData() async {
    try {
      final user = await _userService.getUserById(userId); // Récupérer l'utilisateur
      setState(() {
  ...
  Future<void> _updateUser() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      final updatedUser = UserModel(
        ...
        motDePasse: '', // On ne met pas à jour le mot de passe ici, à moins qu'il ne soit modifié
        telephone: _telephoneController.text,
        statut: _statutController.text,
        groupe: _groupeController.text,
        photo: '', // Peut être mis à jour si une nouvelle photo est choisie
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: Text('Mettre à jour l\'utilisateur')),
      body: Padding(
          ...
          child: ListView(
            children: [
              ...
              Padding(
                ...
                child: Text(_statutController.text, style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              ),
              Text('Groupe(s):'),
              Padding(
                padding: const EdgeInsets.symmetric(vertical: 8.0),
                child: Text(_groupeController.text, style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              ),
              SizedBox(height: 20),
              ElevatedButton(
